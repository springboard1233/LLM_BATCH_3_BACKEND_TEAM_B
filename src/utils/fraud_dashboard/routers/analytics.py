...
# # --- FIX PYTHON PATH (Short & Reliable) ---
# current_dir = os.path.dirname(os.path.abspath(__file__))
# project_root = os.path.abspath(os.path.join(current_dir, "../../../.."))
# if project_root not in sys.path:
#     sys.path.insert(0, project_root)
...
#     collection = get_collection("transactions")
# except Exception as e:
#     print(f"[CRITICAL] Failed to load 'transactions' collection: {e}")
#     collection = None
...
#                 "_id": None,
#                 "mobile_fraud": {
#                     "$sum": {"$cond": [{"$and": [{"$eq": ["$is_fraud", 1]}, {"$eq": ["$channel_mobile", 1]}]}, 1, 0]}
#                 },
#                 "atm_fraud": {
#                     "$sum": {"$cond": [{"$and": [{"$eq": ["$is_fraud", 1]}, {"$eq": ["$channel_atm", 1]}]}, 1, 0]}
#                 },
#                 "pos_fraud": {
#                     "$sum": {"$cond": [{"$and": [{"$eq": ["$is_fraud", 1]}, {"$eq": ["$channel_pos", 1]}]}, 1, 0]}
#                 },
#                 "web_fraud": {
#                     "$sum": {"$cond": [{"$and": [{"$eq": ["$is_fraud", 1]}, {"$eq": ["$channel_web", 1]}]}, 1, 0]}
#                 },
...
# # ---------------------------------------------------------
# # FIX PYTHON PATH (Correct & Minimal)
# # ---------------------------------------------------------
# current_dir = os.path.dirname(os.path.abspath(__file__))
# project_root = os.path.abspath(os.path.join(current_dir, "../../../.."))
# if project_root not in sys.path:
...
# # Import MongoDB collection getter
# from src.utils.fraud_dashboard.database import get_collection

# # Router
# router = APIRouter(prefix="/analytics", tags=["Analytics"])

# # Load 'transactions' collection safely
# try:
#     collection = get_collection("transactions")
# except Exception as e:
#     print(f"[CRITICAL] Could not connect to transactions collection: {e}")
#     collection = None
